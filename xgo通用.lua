			...
			if not _G.YARHM.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", "")) then
				...
				newFloatingButton.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton2 then
						FUNCTIONSmodule.notification("按键绑定 " .. string.gsub(buttonname, "_", "") .. "到... ")
						local keytobind
						local result
						repeat
...
local function IYGQL_fake_script() -- Fake Script: ServerStorage.YARHM.Init
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			print("[YARHM] - 使用get_hidden_gui进行反检测.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			print("[YARHM] - 使用syn.protect_gui进行反检测.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			print("[YARHM] - 使用RobloxGui进行抗检测.")
		else
			warn("[YARHM] - 使用CoreGui作为反检测. 这是最基本的覆盖范围，仍然可以被检测到.")
		end
	end)
	
	print("[YARHM] - Yarhm现在进来了 " .. tostring(script.Parent:GetFullName()))
	if not s then
		warn("[YARHM] - 反检测的尝试失败.使用CoreGui作为反检测.")
		warn(e)	
	end
	
	printidentity("[YARHM] - 您的执行者级别（身份）是")
	
	local getExeName = identifyexecutor or getexecutorname or function() return "另一个Roblox执行程序V1.2" end
	print("[YARHM] - 你的执行官是 " .. getExeName())
	...
	require(script.Parent.FUNCTIONS).notification("欢迎使用xgo通用.")
...
local function FDAOTTY_fake_script() -- Fake Script: ServerStorage.YARHM.Flee the Facility
	...
	local function reloadESP()
		...
		for _, i in ipairs(workspace:GetChildren()) do
			if i.Name == "透视玩家" and not module.players then
				i:Destroy()
			end
	
			if i.Name == "PC亮点" and not module.pcs then
				i:Destroy()
			end
	
			if i.Name == "豆荚突出" and not module.pods then
				i:Destroy()
			end
	
			if i.Name == "退出亮点" and not module.exits then
				i:Destroy()
			end
		end
	...
	task.spawn(function() 
		...
		OldNameCall = hookmetamethod(game, "__namecall", function(Self, ...)
			...
			if NamecallMethod == "FireServer" and Args[1] == "设置Player Minigame结果" and antifail then
				print("minigame结果 - 拦截结果为true")
				Args[2] = true
			end
	...
	module[6] = {
		Type = "Input",
		Args = {"输入拐角","设置和锁定", function(Self, text)
...
local function DXIEA_fake_script() -- Fake Script: ServerStorage.YARHM.Murder Mystery 2
	...
	if not game.ReplicatedStorage:WaitForChild("Remotes", 10) then
		fu.dialog("不是mm2","看起来像这个游戏不是mm2。 无论如何你想加载模块吗？", {"Load", "No"})
	
		if fu.waitfordialog() == "No" then
			fu.closedialog()
			fu.notification("MM2将不会加载，直到您重新加入.", Color3.fromRGB(255, 0, 0), "x")
			return
		end	
		fu.closedialog()
	else
